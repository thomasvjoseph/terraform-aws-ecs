...
resource "aws_appautoscaling_target" "ecs_target" {
  for_each           = { for k, v in var.ecs_resources : k => v if v.enable_autoscaling }
  max_capacity       = each.value.asg_max_size
  min_capacity       = each.value.asg_min_size
...
resource "aws_appautoscaling_policy" "ecs_policy_memory" {
  for_each           = { for k, v in var.ecs_resources : k => v if v.enable_autoscaling && v.enable_memory_autoscaling }
...
resource "aws_appautoscaling_policy" "ecs_policy_cpu" {
  for_each           = { for k, v in var.ecs_resources : k => v if v.enable_autoscaling && v.enable_cpu_autoscaling }
